 ...
 * Enhanced: Stock checking to avoid out-of-stock alerts
 */
...
/**
 * NEW: Check if a product is in stock by fetching product details
 * Returns: { inStock: boolean, stockInfo: string }
 */
async function checkProductStock(productUrl, cookies) {
    try {
        ...
        // Extract product code from URL (e.g., /p/443321580_blue -> 443321580_blue)
        const productCode = productUrl.split('/p/')[1] || productUrl.split('/').pop();
        
        // Product detail API endpoint
        const detailUrl = `https://www.sheinindia.in/api/products/${productCode}?fields=FULL&store=shein`;
        ...
        if (!response.ok) {
            console.log(`   ⚠️ Stock check failed (${response.status}): ${productCode}`);
            return { inStock: true, stockInfo: 'unknown' }; // Default to in-stock if check fails
        }
        
        const data = await response.json();
        
        // Check various stock indicators in the product detail response
        // Common fields: stock.stockLevel, stock.stockLevelStatus, availableForPickup, etc.
        
        // Method 1: Check stock object
        if (data.stock) {
            if (data.stock.stockLevelStatus === 'outOfStock' || 
                data.stock.stockLevel === 0) {
                return { inStock: false, stockInfo: 'out of stock' };
            }
        }
        
        // Method 2: Check if product is purchasable
        if (data.purchasable === false) {
            return { inStock: false, stockInfo: 'not purchasable' };
        }
        
        // Method 3: Check availability flag
        if (data.available === false) {
            return { inStock: false, stockInfo: 'not available' };
        }
        
        // Method 4: Check variant stock levels
        if (data.variantOptions && Array.isArray(data.variantOptions)) {
            let allOutOfStock = true;
            for (const variant of data.variantOptions) {
                if (variant.stock && variant.stock.stockLevelStatus !== 'outOfStock') {
                    allOutOfStock = false;
                    break;
                }
            }
            if (allOutOfStock) {
                return { inStock: false, stockInfo: 'all variants out of stock' };
            }
        }
        
        // If we reach here, assume in stock
        return { inStock: true, stockInfo: 'in stock' };
        
    } catch (error) {
        console.log(`   ⚠️ Stock check error: ${error.message}`);
        return { inStock: true, stockInfo: 'check failed' }; // Default to in-stock on error
    }
}
...
async function runSniper() {
    ...
    if (newProducts.length > 0) {
        ...
        for (let i = 0; i < newProducts.length; i++) {
            ...
            // Check stock status
            const stockResult = await checkProductStock(product.url, cookies);
            console.log(`   📊 Stock: ${stockResult.stockInfo}`);
            ...
            if (stockResult.inStock) {
                ...
                seen[product.id] = Date.now(); // Still mark as seen to avoid checking again
            }
